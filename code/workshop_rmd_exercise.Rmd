---
title: "Rmd workshop exercise"
author: "Moon Hee Yang"
date: "`r Sys.Date()`"
...
# Display the data 
kable(head(data_ordered))
```

```{r generate table for age and mean value}
...
```{r scatterplot, fig.height=6, fig.width=4}
ggplot(new_meta) +
...
#include will run the code but not display or echo or eval =false could be used, but not run as well.
#to change figure size, use fig.height and fig.width
```
